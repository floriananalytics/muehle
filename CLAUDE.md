...
- Speicherung nur in `localStorage` unter dem Präfix `muehle:`: `settings:<spiel-id>` (mode/level/starter), `stats:<spiel-id>` (w/d/l/h; h = Siege, bei denen ein Zugvorschlag angefordert wurde), `sound`, `lastGame`, `newVersion` (Merker für den Versionshinweis). Alte Schlüssel `stats` und `settings` werden beim Start einmalig nach `:muehle` migriert. Keine Server, kein Tracking.
